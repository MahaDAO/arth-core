...
const { Decimal, UserTrove, ARTH_LIQUIDATION_RESERVE } = require("@mahadao/arth-base");
const { EthersLiquity, EthersLiquityWithStore } = require("@mahadao/arth-ethers");
...
async function tryToLiquidate(liquity) {
  ...
  try {
    ...
    const expectedCompensation = total.collateral
      ...
      .add(ARTH_LIQUIDATION_RESERVE.mul(troves.length));
    ...
    const { collateralGasCompensation, arthGasCompensation, liquidatedAddresses } = receipt.details;
    ...
    const totalCompensation = collateralGasCompensation
      .mul(store.state.price)
      .add(arthGasCompensation);

    success(
      `Received ${bold(`${collateralGasCompensation.toString(4)} ETH`)} + ` +
        `${bold(`${arthGasCompensation.toString(2)} ARTH`)} compensation (` +
