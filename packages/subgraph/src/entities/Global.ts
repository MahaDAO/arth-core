...
export function getGlobal(): Global {
  ...
  if (globalOrNull != null) {
    ...
    newGlobal.totalNumberOfMAHAStakes = 0;
    newGlobal.numberOfActiveMAHAStakes = 0;
...
export function updateTotalRedistributed(L_ETH: BigInt, L_ARTHDebt: BigInt): void {
  let global = getGlobal();

  global.rawTotalRedistributedCollateral = L_ETH;
  global.rawTotalRedistributedDebt = L_ARTHDebt;
  global.save();
}
...
export function increaseTotalNumberOfMAHAStakes(): void {
  let global = getGlobal();

  global.totalNumberOfMAHAStakes++;
  global.numberOfActiveMAHAStakes++;
  global.save();
}

export function increaseNumberOfActiveMAHAStakes(): void {
  let global = getGlobal();
...
export function decreaseNumberOfActiveMAHAStakes(): void {
  let global = getGlobal();
...
export function increaseTotalBorrowingFeesPaid(_ARTHFee: BigInt): void {
  let global = getGlobal();
  global.totalBorrowingFeesPaid = global.totalBorrowingFeesPaid.plus(decimalize(_ARTHFee));
  global.save();
}
