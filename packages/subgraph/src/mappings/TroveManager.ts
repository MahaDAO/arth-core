...
export function handleLiquidation(event: Liquidation): void {
  finishCurrentLiquidation(
    ...
    event.params._ARTHGasCompensation
  );
}
...
export function handleRedemption(event: Redemption): void {
  finishCurrentRedemption(
    event,
    event.params._attemptedARTHAmount,
    event.params._actualARTHAmount,
...
export function handleLTermsUpdated(event: LTermsUpdated): void {
  updateTotalRedistributed(event.params._L_ETH, event.params._L_ARTHDebt);
}
