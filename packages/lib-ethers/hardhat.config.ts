...
import { _connectToContracts, _ARTHDeploymentJSON, _priceFeedIsTestnet } from "./src/contracts";
...
task("deploy", "Deploys the contracts to the network")
  ...
  .addOptionalParam(
    "createUniswapPair",
    "Create a real Uniswap v2 WETH-ARTH pair instead of a mock ERC20 token",
    undefined,
    types.boolean
  )
  .setAction(
    async ({ channel, gasPrice, useRealPriceFeed, createUniswapPair }: DeployParams, env) => {
      ...
      const deployment = await env.deployARTH(deployer, useRealPriceFeed, wethAddress, overrides);
