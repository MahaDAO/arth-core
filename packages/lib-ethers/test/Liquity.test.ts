...
describe("EthersARTH", () => {
  ...
  describe("StabilityPool", async() => {
    ...
    const initialTroveOfDepositor = await Trove.create({
    ...
    const troveWithVeryLowICR = await Trove.create({
  ...
  describe("Redemption", () => {
    ...
    it("should redeem some ARTH after the bootstrap phase", async () => {
      ...
      const expectedTotal = await Trove.create({ depositCollateral: 0, borrowARTH: 0 })
      for(let i = 0; i < troveCreations.length; i++) {
        const t = await Trove.create(troveCreations[i])
        expectedTotal.add(t);
      }
    ...
    it("borrowing rate should be maxed out now", async () => {
      ...
      expect(newTrove).to.deep.equal(
        (await Trove.create(troveCreations[0])).adjust({ borrowARTH }, await BorrowingRate.maxBorrowingRate(deployment.addresses.governance, deployer))
      );
    });
  });
