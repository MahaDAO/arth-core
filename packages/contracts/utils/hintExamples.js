...
async function main() {
  ...
  const ARTHAmount = toBN(toWei("2500")); // borrower wants to withdraw 2500 ARTH
  ...
  const liquidationReserve = await troveManager.ARTH_GAS_COMPENSATION();
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(ARTHAmount);

  // Total debt of the new trove = ARTH amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = ARTHAmount.add(expectedFee).add(liquidationReserve);
  ...
  await borrowerOperations.openTrove(maxFee, ARTHAmount, upperHint, lowerHint, ZERO_ADDRESS, { value: ETHColl });

  // --- adjust trove ---

  const collIncrease = toBN(toWei("1")); // borrower wants to add 1 ETH
  const ARTHRepayment = toBN(toWei("230")); // borrower wants to repay 230 ARTH

  // Get trove's current debt and coll
  const { 0: debt, 1: coll } = await troveManager.getEntireDebtAndColl(borrower);

  const newDebt = debt.sub(ARTHRepayment);
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, ARTHRepayment, false, upperHint, lowerHint, {
    value: collIncrease
  });
  ...
  const redemptionhint = await hintHelpers.getRedemptionHints(ARTHAmount, price, 50);

  const {
    0: firstRedemptionHint,
    1: partialRedemptionNewICR,
    2: truncatedARTHAmount
  } = redemptionhint;
  ...
  /* Finally, perform the on-chain redemption, passing the truncated ARTH amount, the correct hints, and the expected
