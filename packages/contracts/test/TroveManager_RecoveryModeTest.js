...
contract("TroveManager - in Recovery Mode", async accounts => {
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1ETH:150ARTH, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100ARTH, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 ARTH of debt, bringing his ICR to 210%
    ...
    // --- TEST ---
    // price drops to 1ETH:100ARTH, reducing TCR to 117%
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 ARTH of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(150, 16)),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390ARTH to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice });
    ...
    // check pool ARTH before liquidation
    const stabilityPoolARTH_Before = (await stabilityPool.getTotalARTHDeposits()).toString();
    assert.equal(stabilityPoolARTH_Before, "390000000000000000000");
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 ARTH.

    With 390 ARTH in the StabilityPool, 390 ARTH should be offset with the pool, leaving 0 in the pool.

    Stability Pool rewards for alice should be:
    ARTHLoss: 390ARTH
    ETHGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 ARTH and 4.074525 ether, the remainders - 1610 ARTH and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner });

    const aliceDeposit = await stabilityPool.getCompoundedARTHDeposit(alice);
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 ARTH and 16.82 ether are distributed.
    ...
    L_ARTHDebt = 1610 / 6 = 268.333 ARTH
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
    const L_ARTHDebt = (await troveManager.L_ARTHDebt()).toString();
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool ARTH is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ARTH > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraARTHAmount: dec(250, 18),
      extraParams: { from: bob }
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Alice deposits ARTH in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 ARTH, Alice sole depositor.
    As liquidated debt (250 ARTH) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240ARTH
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether

    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedARTHDeposit(alice);
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool ARTH > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(220, 16)),
      extraARTHAmount: dec(250, 18),
      extraParams: { from: bob }
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ARTH > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ARTH > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ARTH > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ARTH > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 ARTH, CR of 200%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ARTH < liquidated debt: Trove remains active", async () => {
    ...
    // Alice deposits 1490 ARTH in the Stability Pool
    ...
    /* Since the pool only contains 100 ARTH, and Bob's pre-liquidation debt was 250 ARTH,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ARTH < liquidated debt: Trove remains in TroveOwners array", async () => {
    ...
    // Alice deposits 100 ARTH in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ARTH < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 ARTH) is larger than all ARTH in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ARTH < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ARTH < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    const L_ARTHDebt_After = (await troveManager.L_ARTHDebt()).toString();
    const L_ETH_After = (await troveManager.L_ETH()).toString();

    assert.equal(L_ARTHDebt_After, "0");
    assert.equal(L_ETH_After, "0");
  });

  it("liquidate(), with ICR > 110%, and StabilityPool ARTH < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 ARTH, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraARTHAmount: dec(1500, 18),
      extraParams: { from: alice }
    ...
    //Check SP ARTH has been completely emptied
    assert.equal((await stabilityPool.getTotalARTHDeposits()).toString(), dec(100, 18));
    ...
    // Alice provides another 50 ARTH to pool
    ...
    //Check SP ARTH has been completely emptied
    assert.equal((await stabilityPool.getTotalARTHDeposits()).toString(), dec(150, 18));
  ...
  it("liquidate() with ICR > 110%, and StabilityPool ARTH < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 ARTH to the SP
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraARTHAmount: dec(50, 18),
      extraParams: { from: whale }
    });
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 ETH and a debt of 62 ARTH, and provides 10 ARTH to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 ARTH in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    //Dennis provides 200 ARTH to SP
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({
      ICR: toBN(dec(220, 16)),
      extraARTHAmount: dec(1000, 18),
      extraParams: { from: whale }
    });

    const { arthAmount: A_arthAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraARTHAmount: dec(300, 18),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ARTH of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 ARTH of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraARTHAmount: dec(480, 18),
      extraParams: { from: bob }
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 80 ARTH, ICR = 120% (lowest one)
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 ARTH of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraARTHAmount: dec(90, 18),
      extraParams: { from: bob }
    });
    // Dennis withdraws to 150 ARTH of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 250 ARTH, ICR = 240% (lowest one)
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some ARTH from their Trove, resulting in different ICRs
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1ETH:85ARTH, reducing TCR below 150%
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraARTHAmount: dec(101, 18),
      extraParams: { from: defaulter_1 }
    });
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraARTHAmount: dec(217, 18),
      extraParams: { from: defaulter_2 }
    });
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraARTHAmount: dec(328, 18),
      extraParams: { from: defaulter_3 }
    });
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraARTHAmount: dec(431, 18),
    ...
    // Check Stability Pool has 500 ARTH
  ...
  it("liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty", async () => {
    ...
    // Confirm 0 ARTH in Stability Pool
    assert.equal((await stabilityPool.getTotalARTHDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    // Confirm ARTH in Stability Pool
    assert.equal((await stabilityPool.getTotalARTHDeposits()).toString(), spDeposit.toString());
  ...
  it("liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(
      await troveManager.getPendingARTHDebtReward(alice)
    );
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { arthAmount: arthAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis }
    });
    const { arthAmount: arthAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin }
    });
    const { arthAmount: arthAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy }
    });
    ...
    assert.equal((await arthToken.balanceOf(dennis)).toString(), arthAmountD);
    assert.equal((await arthToken.balanceOf(erin)).toString(), arthAmountE);
    assert.equal((await arthToken.balanceOf(freddy)).toString(), arthAmountF);
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides ARTH to the SP
    const { arthAmount: W_arthAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraARTHAmount: dec(4000, 18),
      extraParams: { from: whale }
    });
    await stabilityPool.provideToSP(W_arthAmount, ZERO_ADDRESS, { from: whale });

    const {
      arthAmount: A_arthAmount,
    ...
    await stabilityPool.provideToSP(A_arthAmount, ZERO_ADDRESS, { from: alice });
    await stabilityPool.provideToSP(B_arthAmount, ZERO_ADDRESS, { from: bob });

    const totalDeposit = W_arthAmount.add(A_arthAmount).add(B_arthAmount);
    ...
    // Check ARTH in Pool
    assert.equal((await stabilityPool.getTotalARTHDeposits()).toString(), totalDeposit);
    ...
    Total ARTH in Pool: 680 ARTH

    Then, liquidation hits A,B,C:

    Total liquidated debt = 100 + 300 + 100 = 500 ARTH
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale ARTH Loss: 500 * (400/680) = 294.12 ARTH
    Alice ARTH Loss:  500 *(40/680) = 29.41 ARTH
    Bob ARTH Loss: 500 * (240/680) = 176.47 ARTH

    Whale remaining deposit: (400 - 294.12) = 105.88 ARTH
    Alice remaining deposit: (40 - 29.41) = 10.59 ARTH
    Bob remaining deposit: (240 - 176.47) = 63.53 ARTH

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 ARTH
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedARTHDeposit(whale)).toString();
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_arthAmount.sub(liquidatedDebt.mul(W_arthAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_arthAmount.sub(liquidatedDebt.mul(A_arthAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_arthAmount.sub(liquidatedDebt.mul(B_arthAmount).div(totalDeposit))
      ),
      100000
    );
    ...
    assert.isAtMost(
      th.getDifference(
        alice_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(A_arthAmount).div(totalDeposit)
      ),
      2000
    );
    assert.isAtMost(
      th.getDifference(
        bob_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(B_arthAmount).div(totalDeposit)
      ),
      2000
    );
    ...
    assert.isAtMost(th.getDifference(total_ARTHinSP, totalDeposit.sub(liquidatedDebt)), 1000);
  ...
  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
    ...
    // Check ARTH and ETH in Pool  before
    ...
    assert.equal(ARTHinSP_Before, dec(800, 18));
    ...
    // Check ARTH and ETH in Pool after
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    That leaves 50 ARTH in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10);
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 ARTH in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated.
    That leaves 50 ARTH in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10);

    // Expect system debt reduced by 203 ARTH and system coll 2.3 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl();
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated.
    That leaves 50 ARTH in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10);

    const [liquidatedDebt, liquidatedColl, collGasComp, arthGasComp] =
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 ARTH and system coll by 2 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl();
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool ARTH > debt to liquidate: can liquidate troves out of order", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice }
    });
    ...
    // Whale provides 1000 ARTH to the SP
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(
      await troveManager.getPendingARTHDebtReward(dennis)
    );
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(
      await troveManager.getPendingARTHDebtReward(bob)
    );
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(
      await troveManager.getPendingARTHDebtReward(carol)
    );
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    // Whale withdraws entire deposit, and re-deposits 132 ARTH
    ...
    // Expected Stability Pool to fully absorb B (92 ARTH + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalARTHDeposits();
    ...
    const stabilityAfter = await stabilityPool.getTotalARTHDeposits();
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    // Whale opens trove and adds 220 ARTH to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)));
    await openTrove({
      ICR: toBN(dec(285, 16)),
      extraARTHAmount: spDeposit,
