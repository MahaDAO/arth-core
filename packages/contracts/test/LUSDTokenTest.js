...
contract("ARTHValuecoin", async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      // mint some tokens
      if (withProxy) {
        await arthTokenOriginal.unprotectedMint(arthTokenTester.getProxyAddressFromUser(alice), 150);
        await arthTokenOriginal.unprotectedMint(arthTokenTester.getProxyAddressFromUser(bob), 100);
        await arthTokenOriginal.unprotectedMint(arthTokenTester.getProxyAddressFromUser(carol), 50);
      } else {
    ...
    it("name(): returns the token's name", async () => {
      ...
      assert.equal(name, "ARTH Stablecoin");
    });
