  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      if (withProxy) {
        ...
        await deploymentHelper.deployProxyScripts(contracts, MAHAContracts, owner, users);
      }
    ...
    it("withdrawARTH(): borrowing at non-zero base rate sends ARTH fee to MAHA staking contract", async () => {
      ...
      // Check MAHA ARTH balance after has increased
    ...
    it("withdrawARTH(): Borrowing at non-zero base rate increases the MAHA staking contract ARTH fees-per-unit-staked", async () => {
      ...
      // Check MAHA contract ARTH fees-per-unit-staked has increased
    ...
    it("withdrawARTH(): Borrowing at zero base rate changes ARTH fees-per-unit-staked", async () => {
      ...
      // A artificially receives MAHA, then stakes it
      ...
      // Check MAHA ARTH balance after > 0
      const F_ARTH_After = await mahaStaking.F_ARTH();
    ...
    it("adjustTrove(): Borrowing at zero base rate changes ARTH balance of MAHA staking contract", async () => {
      ...
      const mahaStaking_ARTHBalance_Before = await arthToken.balanceOf(mahaStaking.address);
      assert.isTrue(mahaStaking_ARTHBalance_Before.gt(toBN("0")));
