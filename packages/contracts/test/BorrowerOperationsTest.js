...
contract("BorrowerOperations", async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      contracts = await deploymentHelper.deployARTHTokenTester(contracts, owner);
    ...
    it("addColl(): reverts when top-up would leave trove with ICR < MCR", async () => {
      ...
      const collTopUp = 0; // 0 wei top up
    ...
    it("withdrawColl(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      ...
      const collWithdrawal = 0; // 1 wei withdrawal
    ...
    it("withdrawColl(): reduces the Trove's collateral by the correct amount", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice);
      console.log("aliceCollBefore", aliceCollBefore.toString())
    ...
    it("withdrawARTH(): increases ARTH debt in ActivePool by correct amount", async () => {
      ...
      th.assertIsApproximatelyEqual(
        ...
        activePool_ARTH_Before.add(toBN(dec(1000, 18)))
      );
    });
    ...
    it("repayARTH(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      ...
      const repayTxA = await borrowerOperations.repayARTH(2, A, A, { from: A });
      assert.isTrue(repayTxA.receipt.status);

      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, ZERO_ADDRESS, { from: B, value: dec(100, 30) });
    ...
    /*it("closeTrove(): reverts if borrower has insufficient ARTH balance to repay his entire debt", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(1500, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
        extraARTHAmount: toBN(dec(5, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: B }
      });
      ...
      console.log("**dev----balanceOf", B_ARTHBal.toString())
      assert.isTrue(B_ARTHBal.lt(B_troveDebt));
      console.log("**dev----get Debt", (await troveManager.getTroveDebt(B)).toString())
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      assert.isTrue(carolICR.gt(toBN(dec(15, 16))));
    });
    ...
    if (!withProxy) {
      it("closeTrove(): fails if owner cannot receive ETH", async () => {
        ...
        await borrowerOperations.openTrove(th._100pct, dec(100000, 18), alice, alice, ZERO_ADDRESS, { from: alice, value: dec(1000, 18) });
