...
contract('Pool Manager: Sum-Product rounding errors', async accounts => {
  ...
  it.skip("Rounding errors: 100 deposits of 100ARTH into SP, then 200 liquidations of 49ARTH", async () => {
    const owner = accounts[0]
    const depositors = accounts.slice(1, 101)
    const defaulters = accounts.slice(101, 301)

    for (let account of depositors) {
      await openTrove({ extraARTHAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: account } })
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalARTHDeposits()
    const SP_ETH = await stabilityPool.getETH()
    const compoundedDeposit = await stabilityPool.getCompoundedARTHDeposit(depositors[0])
