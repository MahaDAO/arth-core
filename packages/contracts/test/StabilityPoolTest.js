...
contract("StabilityPool", async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    // --- MAHA functionality ---
    it("provideToSP(), new deposit: when SP > 0, triggers MAHA reward event - increases the sum G", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") }
    ...
    it("provideToSP(), new deposit: depositor does not receive any MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") }
      ...
      // Get A, B, C MAHA balances before and confirm they're zero
      const A_MAHABalance_Before = await mahaToken.balanceOf(A);
      const B_MAHABalance_Before = await mahaToken.balanceOf(B);

      assert.equal(A_MAHABalance_Before, "0");
      assert.equal(B_MAHABalance_Before, "0");
      ...
      // Get A, B, C MAHA balances after, and confirm they're still zero
      const A_MAHABalance_After = await mahaToken.balanceOf(A);
      const B_MAHABalance_After = await mahaToken.balanceOf(B);

      assert.equal(A_MAHABalance_After, "0");
      assert.equal(B_MAHABalance_After, "0");
    });

    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get A, B, C MAHA balances before and confirm they're non-zero
      const A_MAHABalance_Before = await mahaToken.balanceOf(A);
      const B_MAHABalance_Before = await mahaToken.balanceOf(B);
      assert.isTrue(A_MAHABalance_Before.gt(toBN("0")));
      assert.isTrue(B_MAHABalance_Before.gt(toBN("0")));
      ...
      // Get A, B, C MAHA balances after, and confirm they have not changed
      const A_MAHABalance_After = await mahaToken.balanceOf(A);
      const B_MAHABalance_After = await mahaToken.balanceOf(B);

      assert.isTrue(A_MAHABalance_After.eq(A_MAHABalance_Before));
      assert.isTrue(B_MAHABalance_After.eq(B_MAHABalance_Before));
    });

    it("provideToSP(), new eligible deposit: tagged front end receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get F1, F2, F3 MAHA balances before, and confirm they're zero
      const frontEnd_1_MAHABalance_Before = await mahaToken.balanceOf(frontEnd_1);
      const frontEnd_2_MAHABalance_Before = await mahaToken.balanceOf(frontEnd_2);
      const frontEnd_3_MAHABalance_Before = await mahaToken.balanceOf(frontEnd_3);

      assert.equal(frontEnd_1_MAHABalance_Before, "0");
      assert.equal(frontEnd_2_MAHABalance_Before, "0");
      assert.equal(frontEnd_3_MAHABalance_Before, "0");

      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);

      // console.log(`MAHASupplyCap before: ${await communityIssuance.MAHASupplyCap()}`)
      // console.log(`totalMAHAIssued before: ${await communityIssuance.totalMAHAIssued()}`)
      // console.log(`MAHA balance of CI before: ${await mahaToken.balanceOf(communityIssuance.address)}`)
      ...
      // console.log(`MAHASupplyCap after: ${await communityIssuance.MAHASupplyCap()}`)
      // console.log(`totalMAHAIssued after: ${await communityIssuance.totalMAHAIssued()}`)
      // console.log(`MAHA balance of CI after: ${await mahaToken.balanceOf(communityIssuance.address)}`)

      // Get F1, F2, F3 MAHA balances after, and confirm they have increased
      const frontEnd_1_MAHABalance_After = await mahaToken.balanceOf(frontEnd_1);
      const frontEnd_2_MAHABalance_After = await mahaToken.balanceOf(frontEnd_2);
      const frontEnd_3_MAHABalance_After = await mahaToken.balanceOf(frontEnd_3);

      assert.isTrue(frontEnd_1_MAHABalance_After.gt(frontEnd_1_MAHABalance_Before));
      assert.isTrue(frontEnd_2_MAHABalance_After.gt(frontEnd_2_MAHABalance_Before));
      assert.isTrue(frontEnd_3_MAHABalance_After.gt(frontEnd_3_MAHABalance_Before));
    });
    ...
    it("provideToSP(), topup: triggers MAHA reward event - increases the sum G", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Expect G has increased from the MAHA reward event triggered by B's topup
      assert.isTrue(G_After.gt(G_Before));
    });
    ...
    it("provideToSP(), topup: depositor receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get A, B, C MAHA balance before
      const A_MAHABalance_Before = await mahaToken.balanceOf(A);
      const B_MAHABalance_Before = await mahaToken.balanceOf(B);
      const C_MAHABalance_Before = await mahaToken.balanceOf(C);
      ...
      // Get MAHA balance after
      const A_MAHABalance_After = await mahaToken.balanceOf(A);
      const B_MAHABalance_After = await mahaToken.balanceOf(B);
      const C_MAHABalance_After = await mahaToken.balanceOf(C);

      // Check MAHA Balance of A, B, C has increased
      assert.isTrue(A_MAHABalance_After.gt(A_MAHABalance_Before));
      assert.isTrue(B_MAHABalance_After.gt(B_MAHABalance_Before));
      assert.isTrue(C_MAHABalance_After.gt(C_MAHABalance_Before));
    });

    it("provideToSP(), topup: tagged front end receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get front ends' MAHA balance before
      const F1_MAHABalance_Before = await mahaToken.balanceOf(frontEnd_1);
      const F2_MAHABalance_Before = await mahaToken.balanceOf(frontEnd_2);
      const F3_MAHABalance_Before = await mahaToken.balanceOf(frontEnd_3);
      ...
      // Get front ends' MAHA balance after
      const F1_MAHABalance_After = await mahaToken.balanceOf(A);
      const F2_MAHABalance_After = await mahaToken.balanceOf(B);
      const F3_MAHABalance_After = await mahaToken.balanceOf(C);

      // Check MAHA Balance of front ends has increased
      assert.isTrue(F1_MAHABalance_After.gt(F1_MAHABalance_Before));
      assert.isTrue(F2_MAHABalance_After.gt(F2_MAHABalance_Before));
      assert.isTrue(F3_MAHABalance_After.gt(F3_MAHABalance_Before));
    });
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_MAHABalBefore = await mahaToken.balanceOf(A);

      // Check Alice has gains to withdraw
      const A_pendingETHGain = await stabilityPool.getDepositorETHGain(A);
      const A_pendingMAHAGain = await stabilityPool.getDepositorMAHAGain(A);
      assert.isTrue(A_pendingETHGain.gt(toBN("0")));
      assert.isTrue(A_pendingMAHAGain.gt(toBN("0")));
      ...
      const A_MAHABalAfter = await mahaToken.balanceOf(A);
      const A_MAHABalDiff = A_MAHABalAfter.sub(A_MAHABalBefore);

      // Check A's ETH and MAHA balances have increased correctly
      assert.isTrue(A_ETHBalAfter.sub(A_expectedBalance).eq(A_pendingETHGain));
      assert.isAtMost(th.getDifference(A_MAHABalDiff, A_pendingMAHAGain), 1000);
    });
    ...
    // --- MAHA functionality ---
    it("withdrawFromSP(): triggers MAHA reward event - increases the sum G", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(1, 24)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawFromSP(), partial withdrawal: depositor receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      // Fast-forward time and make a second deposit, to trigger MAHA reward and make G > 0
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E });
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      //  SETUP: Execute a series of operations to trigger MAHA and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): triggers MAHA reward event - increases the sum G", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawETHGainToTrove(), eligible deposit: depositor receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawETHGainToTrove(), eligible deposit: tagged front end receives MAHA rewards", async () => {
      await openTrove({
        extraARTHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get front ends' MAHA balance after
      const F1_MAHABalance_After = await mahaToken.balanceOf(frontEnd_1);
      const F2_MAHABalance_After = await mahaToken.balanceOf(frontEnd_2);
      const F3_MAHABalance_After = await mahaToken.balanceOf(frontEnd_3);
    ...
    it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
      ...
      // fastforward time, and E makes a deposit, creating MAHA rewards for all
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);
      await openTrove({
        extraARTHAmount: toBN(dec(3000, 18)),
        ICR: toBN(dec(2, 18)),
