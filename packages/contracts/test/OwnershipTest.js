...
contract("All Liquity functions with onlyOwner modifier", async accounts => {
  ...
  describe("CommunityIssuance", async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      const params = [mahaToken.address, stabilityPool.address];
  ...
  describe("MAHAStaking", async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      await testSetAddresses(mahaStaking, 5);
    });
  });

  describe("LockupContractFactory", async accounts => {
    it("setMAHAAddress(): reverts when called by non-owner, with wrong address, or twice", async () => {
      await th.assertRevert(
        lockupContractFactory.setMAHATokenAddress(mahaToken.address, { from: alice })
      );

      const params = [mahaToken.address];

      // Attempt to use zero address
      await testZeroAddress(lockupContractFactory, params, "setMAHATokenAddress");
      // Attempt to use non contract
      await testNonContractAddress(lockupContractFactory, params, "setMAHATokenAddress");

      // Owner can successfully set any address
      const txOwner = await lockupContractFactory.setMAHATokenAddress(mahaToken.address, {
        from: owner
      });
      ...
      // fails if called twice
      await th.assertRevert(
        lockupContractFactory.setMAHATokenAddress(mahaToken.address, { from: owner })
      );
    });
  });
});
