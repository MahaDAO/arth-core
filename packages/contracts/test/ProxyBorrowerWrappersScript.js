...
contract("BorrowerWrappers", async accounts => {
  ...
  it("claimCollateralAndOpenTrove(): reverts if nothing to claim", async () => {
    ...
    // alice opens Trove
    const { arthAmount, collateral } = await openTrove({
      ICR: toBN(dec(15, 17)),
    ...
    await assertRevert(
      borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, arthAmount, alice, alice, {
        from: alice
      }),
    ...
    th.assertIsApproximatelyEqual(await arthToken.balanceOf(proxyAddress), arthAmount);
  ...
  it("claimCollateralAndOpenTrove(): without sending any value", async () => {
    ...
    const {
      ...
      collateral
    } = await openTrove({ extraARTHAmount: 0, ICR: toBN(dec(3, 18)), extraParams: { from: alice } });
    // Whale opens Trove
    await openTrove({
      extraARTHAmount: redeemAmount,
      ICR: toBN(dec(5, 18)),
    ...
    // alice claims collateral and re-opens the trove
    await borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, arthAmount, alice, alice, {
      from: alice
    });

    assert.equal(await web3.eth.getBalance(proxyAddress), "0");
    th.assertIsApproximatelyEqual(await collSurplusPool.getCollateral(proxyAddress), "0");
    th.assertIsApproximatelyEqual(await arthToken.balanceOf(proxyAddress), arthAmount.mul(toBN(2)));
    assert.equal(await troveManager.getTroveStatus(proxyAddress), 1);
  ...
  it("claimSPRewardsAndRecycle(): only owner can call it", async () => {
    // Whale opens Trove
    await openTrove({
      extraARTHAmount: toBN(dec(1850, 18)),
      ICR: toBN(dec(2, 18)),
      extraParams: { from: whale }
    });
    ...
    // alice opens trove and provides 150 ARTH to StabilityPool
    await openTrove({ extraARTHAmount: toBN(dec(150, 18)), extraParams: { from: alice } });
  ...
  it("claimSPRewardsAndRecycle():", async () => {
    ...
    // Defaulter Trove opened
    const { arthAmount, netDebt, collateral } = await openTrove({
      ICR: toBN(dec(210, 16)),
    ...
    // Alice ARTHLoss is ((150/2500) * liquidatedDebt)
    const totalDeposits = whaleDeposit.add(aliceDeposit);
    const expectedARTHLoss_A = liquidatedDebt_1.mul(aliceDeposit).div(totalDeposits);

    const expectedCompoundedARTHDeposit_A = toBN(dec(150, 18)).sub(expectedARTHLoss_A);
    ...
    const arthBalanceBefore = await arthToken.balanceOf(alice);
    const troveDebtBefore = await troveManager.getTroveDebt(alice);
    const mahaBalanceBefore = await mahaToken.balanceOf(alice);
    const ICRBefore = await troveManager.getCurrentICR(alice, price);
    const depositBefore = (await stabilityPool.deposits(alice))[0];
    const stakeBefore = await mahaStaking.stakes(alice);

    const proportionalARTH = expectedETHGain_A.mul(price).div(ICRBefore);
    const borrowingRate = await troveManagerOriginal.getBorrowingRateWithDecay();
    const netDebtChange = proportionalARTH.mul(mv._1e18BN).div(mv._1e18BN.add(borrowingRate));

    // to force MAHA issuance
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider);

    const expectedMAHAGain_A = toBN("50373424199406504708132");
    ...
    const arthBalanceAfter = await arthToken.balanceOf(alice);
    const troveDebtAfter = await troveManager.getTroveDebt(alice);
    const mahaBalanceAfter = await mahaToken.balanceOf(alice);
    const ICRAfter = await troveManager.getCurrentICR(alice, price);
    const depositAfter = (await stabilityPool.deposits(alice))[0];
    const stakeAfter = await mahaStaking.stakes(alice);

    // check proxy balances remain the same
    assert.equal(ethBalanceAfter.toString(), ethBalanceBefore.toString());
    assert.equal(arthBalanceAfter.toString(), arthBalanceBefore.toString());
    assert.equal(mahaBalanceAfter.toString(), mahaBalanceBefore.toString());
    // check trove has increased debt by the ICR proportional amount to ETH gain
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore.add(proportionalARTH));
    ...
    th.assertIsApproximatelyEqual(
      depositAfter,
      depositBefore.sub(expectedARTHLoss_A).add(netDebtChange)
    );
    // check maha balance remains the same
    th.assertIsApproximatelyEqual(mahaBalanceAfter, mahaBalanceBefore);

    // MAHA staking
    th.assertIsApproximatelyEqual(stakeAfter, stakeBefore.add(expectedMAHAGain_A));
  ...
  it("claimStakingGainsAndRecycle(): only owner can call it", async () => {
    ...
    // mint some MAHA
    await mahaTokenOriginal.unprotectedMint(
    ...
    // stake MAHA
    await mahaStaking.stake(dec(1850, 18), { from: whale });
    await mahaStaking.stake(dec(150, 18), { from: alice });

    // Defaulter Trove opened
    const { arthAmount, netDebt, totalDebt, collateral } = await openTrove({
      ICR: toBN(dec(210, 16)),
      extraParams: { from: defaulter_1 }
    });
  ...
  it("claimStakingGainsAndRecycle(): reverts if user has no trove", async () => {
    ...
    const borrowingFee = netDebt.sub(arthAmount);

    // Alice ARTH gain is ((150/2000) * borrowingFee)
    const expectedARTHGain_A = borrowingFee.mul(toBN(dec(150, 18))).div(toBN(dec(2000, 18)));
    ...
    th.assertIsApproximatelyEqual(mahaBalanceBefore, mahaBalanceAfter);
    // MAHA staking
  ...
  it("claimStakingGainsAndRecycle(): with only ETH gain", async () => {
    ...
    const proportionalARTH = expectedETHGain_A.mul(price).div(ICRBefore);
    const borrowingRate = await troveManagerOriginal.getBorrowingRateWithDecay();
    const netDebtChange = proportionalARTH
      .mul(toBN(dec(1, 18)))
      .div(toBN(dec(1, 18)).add(borrowingRate));

    const expectedMAHAGain_A = toBN("839557069990108416000000");

    const proxyAddress = borrowerWrappers.getProxyAddressFromUser(alice);
    // Alice claims staking rewards and puts them back in the system through the proxy
    await borrowerWrappers.claimStakingGainsAndRecycle(th._100pct, alice, alice, { from: alice });

    // Alice new ARTH gain due to her own Trove adjustment: ((150/2000) * (borrowing fee over netDebtChange))
    const newBorrowingFee = await troveManagerOriginal.getBorrowingFeeWithDecay(netDebtChange);
    const expectedNewARTHGain_A = newBorrowingFee.mul(toBN(dec(150, 18))).div(toBN(dec(2000, 18)));
    ...
    assert.equal(ethBalanceAfter.toString(), ethBalanceBefore.toString());
    assert.equal(mahaBalanceAfter.toString(), mahaBalanceBefore.toString());
    // check proxy arth balance has increased by own adjust trove reward
    th.assertIsApproximatelyEqual(arthBalanceAfter, arthBalanceBefore.add(expectedNewARTHGain_A));
    // check trove has increased debt by the ICR proportional amount to ETH gain
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore.add(proportionalARTH), 10000);
    // check trove has increased collateral by the ETH gain
    th.assertIsApproximatelyEqual(troveCollAfter, troveCollBefore.add(expectedETHGain_A));
    // check that ICR remains constant
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore);
  ...
  it("claimStakingGainsAndRecycle(): with only ARTH gain", async () => {
    const price = toBN(dec(200, 18));
    ...
    assert.equal(mahaBalanceAfter.toString(), mahaBalanceBefore.toString());
    // check proxy arth balance has increased by own adjust trove reward
    th.assertIsApproximatelyEqual(arthBalanceAfter, arthBalanceBefore);
    // check trove has increased debt by the ICR proportional amount to ETH gain
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore, 10000);
    // check trove has increased collateral by the ETH gain
    th.assertIsApproximatelyEqual(troveCollAfter, troveCollBefore);
    ...
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore);
    // check that Stability Pool deposit
    th.assertIsApproximatelyEqual(depositAfter, depositBefore.add(expectedARTHGain_A), 10000);
    // check maha balance remains the same
    th.assertIsApproximatelyEqual(mahaBalanceBefore, mahaBalanceAfter);
  ...
  it("claimStakingGainsAndRecycle(): with both ETH and ARTH gains", async () => {
    const price = toBN(dec(200, 18));
    ...
    const netDebtChange = proportionalARTH
      .mul(toBN(dec(1, 18)))
      .div(toBN(dec(1, 18)).add(borrowingRate));
    const expectedTotalARTH = expectedARTHGain_A.add(netDebtChange);
    ...
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore);
    // check that Stability Pool deposit
    th.assertIsApproximatelyEqual(depositAfter, depositBefore.add(expectedTotalARTH), 10000);
    // check maha balance remains the same
    th.assertIsApproximatelyEqual(mahaBalanceBefore, mahaBalanceAfter);
