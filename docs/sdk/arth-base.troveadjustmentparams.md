...
[Home](./index.md) &gt; [@mahadao/arth-base](./arth-base.md) &gt; [TroveAdjustmentParams](./arth-base.troveadjustmentparams.md)
...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowARTH` and `repayARTH` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowARTH? </td> <td> T </td> <td> <i>(Optional)</i> The amount of ARTH that's borrowed. </td> </tr>

<tr> <td> repayARTH? </td> <td> T </td> <td> <i>(Optional)</i> The amount of ARTH that's repaid. </td> </tr>
